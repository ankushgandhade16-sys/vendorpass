...
import { Building2, User, Lock as LockIcon, ArrowRight, ArrowLeft, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F6FBF7] flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      <div className="card w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight">{t('wholesaler')} {t('login')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-bold text-emerald-700 mb-2 px-1 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> {t('username')}
            </label>
          ...
          <div>
            <label className="block text-sm font-bold text-amber-700 mb-2 px-1 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400"></span> {t('password')}
            </label>
            <div className="relative group">
        ...
        <p className="text-center mt-10 text-sm text-slate-500 font-medium">
          {t('dontHaveAccount')} <Link to="/wholesaler/register" className="text-emerald-600 font-bold hover:underline">{t('register')} →</Link>
        </p>
      </div>
    </div>
  );
