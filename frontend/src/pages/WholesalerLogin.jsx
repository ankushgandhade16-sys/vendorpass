...
const WholesalerLogin = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-6 relative">
      <Link to="/" className="absolute top-6 left-6 text-slate-500 hover:text-slate-800 flex items-center gap-2">
      ...
      <div className="card w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-slate-900">Wholesaler Login</h2>
          <p className="text-slate-500 mt-2">Access your distribution dashboard</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 px-4 rounded-xl shadow-lg transition-all mt-2">Sign In</button>
        </form>

        <p className="text-center mt-6 text-sm text-slate-600">
          New supplier? <Link to="/wholesaler/register" className="text-emerald-600 font-semibold hover:underline">Register here</Link>
        </p>
      </div>
    </div>
  );
