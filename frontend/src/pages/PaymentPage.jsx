...
const PaymentPage = () => {
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      setStatus('error');
      setErrorMsg(err.response?.data?.msg || t('paymentFailed'));
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#F6FBF7] flex flex-col justify-center items-center p-6 text-center relative overflow-hidden">
        ...
        <div className="card max-w-sm w-full relative z-10 border-emerald-50 scale-110">
          ...
          <h2 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">{t('success')}!</h2>
          <p className="text-slate-600 font-medium mb-8">{t('successfullyTransferred')} <span className="text-emerald-600 font-bold">₹{amount}</span> {t('toVendorWallet')}.</p>
  ...
  return (
    <div className="min-h-screen bg-[#F6FBF7] flex flex-col justify-center items-center p-6 relative overflow-hidden">
      ...
      <div className="absolute top-10 left-10 flex items-center gap-6 z-20">
        <button onClick={() => navigate(-1)} className="text-slate-500 hover:text-slate-900 flex items-center gap-2 font-bold transition-all hover:-translate-x-1">
      ...
      <div className="card w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">{t('securePayment') || 'Secure Payment'}</h2>
          <div className="mt-3 inline-flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('recipientId') || 'Recipient ID'}</span>
        ...
        <form onSubmit={handlePayment} className="space-y-8">
          <div>
            <label className="block text-center text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-6">{t('enterTransferAmount') || 'Enter Amount to Transfer'}</label>
          ...
          >
            {status === 'loading' ? t('sending') || 'Encrypting & Sending...' : (
              <>
                <Send className="w-6 h-6" /> {t('completePayment') || 'Complete Payment'}
              </>
            )}
          </button>
          
          <div className="flex items-center justify-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
            <ShieldCheck className="w-4 h-4 text-emerald-500" /> {t('poweredBy') || 'Powered by VendorPass Secure'}
          </div>
        </form>
      </div>
    </div>
