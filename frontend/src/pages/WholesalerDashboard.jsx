...
import { Users, CreditCard, LogOut, Wallet, Building2, MessageSquare, User, Send, IndianRupee, AlertTriangle } from 'lucide-react';
...
const WholesalerDashboard = () => {
  ...
  const handleStatusUpdate = async (id, status) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`/api/credit/${id}`, { status }, { headers: { 'x-auth-token': token } });
      fetchRequests();
    } catch (err) { alert(err.response?.data?.msg || 'Error updating status'); }
  };

  const startChatWithVendor = async (vendor) => {
    setChatUserId(vendor.user);
        ...
        <div className="flex gap-1 mb-6 bg-white p-1 rounded-xl shadow-sm border border-slate-100 overflow-x-auto">
          {['vendors', 'requests', 'messages'].map(tab => (
            <button key={tab} onClick={() => { setActiveTab(tab); setChatUserId(null); }}
              className={`flex-1 py-2 px-3 rounded-lg font-medium transition text-sm capitalize ${activeTab === tab ? 'bg-emerald-100 text-emerald-700' : 'text-slate-500 hover:bg-slate-50'}`}>
              {tab === 'messages' ? 'Chat' : tab}
            </button>
          ))}
        </div>
                ...
                <div key={r._id} className={`bg-white p-5 rounded-2xl shadow-sm border ${isOverdue ? 'border-red-300 bg-red-50' : 'border-slate-200'}`}>
                  <div className="flex justify-between mb-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Loan: <span className="font-bold text-slate-700">₹{r.amount}</span></p>
                    </div>
                  ...
                  {['Active', 'Overdue', 'Defaulted'].includes(r.status) && (
                    <div className="bg-slate-50 rounded-xl p-3 mb-3 text-sm space-y-1">
                      <div className="flex justify-between"><span className="text-slate-500">Interest</span><span className={isOverdue ? 'text-red-600 font-bold' : ''}>{r.interestRate}%</span></div>
                      ...
                      <div className="flex justify-between"><span className="text-slate-500">Paid</span><span className="text-emerald-600">₹{r.amountPaid || 0}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500">Remaining</span><span className="font-bold">₹{remaining.toFixed(2)}</span></div>
                      <div className="flex justify-between"><span className="text-slate-500">Due Date</span><span className={isOverdue ? 'text-red-600' : ''}>{new Date(r.dueDate).toLocaleDateString()}</span></div>
                      {isOverdue && (
                        <div className="flex items-center gap-2 text-red-600 mt-2">
                          <AlertTriangle className="w-4 h-4" />
                          <span>Overdue! Interest increasing.</span>
                        </div>
                      )}
                    </div>
                  )}
          ...
          <div className="space-y-4">
            ...
            {conversations.length === 0 && (
              <div className="text-center p-8 bg-white rounded-3xl border border-slate-200">
                ...
                <p className="text-sm text-slate-400">Go to the Vendors tab to start chatting!</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'messages' && chatUserId && (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              {chatMessages.map(msg => (
                <div key={msg._id} className={`flex ${msg.sender === user._id ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl ${
                    msg.sender === user._id 
                      ? msg.type === 'payment' ? 'bg-emerald-600 text-white' : 'bg-emerald-600 text-white'
                      : msg.type === 'payment' ? 'bg-emerald-50 border border-emerald-200 text-emerald-800' : 'bg-white border border-slate-200 text-slate-800'
                  }`}>
                    {msg.type === 'payment' && (
            ...
            <div className="p-4 border-t border-slate-100 space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowPayInput(!showPayInput)} className={`p-3 rounded-xl transition border ${showPayInput ? 'bg-emerald-100 border-emerald-300 text-emerald-700' : 'border-slate-200 hover:bg-slate-50 text-slate-500'}`}>
                ...
                <button onClick={sendMessage} className="p-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition">
